...
class MemoryManager:
    ...
    def _init_db(self):
        self.cursor.execute("CREATE TABLE IF NOT EXISTS conversations (id INTEGER PRIMARY KEY AUTOINCREMENT, role TEXT, content TEXT, timestamp DATETIME DEFAULT CURRENT_TIMESTAMP, language TEXT DEFAULT 'ar')")
        self.cursor.execute("CREATE TABLE IF NOT EXISTS user_profile (key TEXT PRIMARY KEY, value TEXT, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP)")
        self.cursor.execute("CREATE TABLE IF NOT EXISTS memories (id INTEGER PRIMARY KEY AUTOINCREMENT, fact TEXT, source_id INTEGER, confidence REAL DEFAULT 0.8)")
        self.conn.commit()
    
    def save_message(self, role, content, language='ar'):
        self.cursor.execute("INSERT INTO conversations (role, content, language) VALUES (?, ?, ?)", (role, content, language))
        self.conn.commit()
    
    def get_recent_context(self, limit=15):
        self.cursor.execute("SELECT role, content, language FROM conversations ORDER BY id DESC LIMIT ?", (limit,))
    ...
    def save_memory(self, fact, confidence=0.8):
        self.cursor.execute("INSERT INTO memories (fact, confidence) VALUES (?, ?)", (fact, confidence))
        self.conn.commit()
    ...
    def get_relevant_memories(self, query, limit=3):
        ...
        self.cursor.execute("SELECT fact FROM memories WHERE " + like_clauses + " LIMIT ?", params + [limit])
        return [r[0] for r in self.cursor.fetchall()]
    
    def forget_memory(self, keyword):
        self.cursor.execute("DELETE FROM memories WHERE fact LIKE ?", ("%" + keyword + "%",))
        self.conn.commit()
...
class AIManager:
    def __init__(self, memory):
        ...
        if not self.api_key:
            raise ValueError("⚠️ خطأ أمني: لم يتم العثور على OPENROUTER_API_KEY. يرجى تعيينه في البيئة.")
        ...
        self.headers = {
            ...
            "HTTP-Referer": "https://riy-agent.app",
            "X-Title": "Riy AI Agent"
        }
    ...
    def get_response(self, prompt, callback):
        def _fetch():
            try:
                ...
                profile_text = "\n".join(["- " + k + ": " + v for k, v in profile.items()]) if profile else "No info"
                ...
                with urllib.request.urlopen(req, timeout=30) as resp:
                    ...
                    Clock.schedule_once(lambda dt: callback(True, result['choices'][0]['message']['content']), 0)
...
class RiyAgentApp(MDApp):
    def build(self):
        ...
        self.stats = MDLabel(text="Memory: " + str(c) + " msg | " + str(m) + " facts", font_style="Caption", theme_text_color="Custom", text_color=(0.7,0.7,0.7,1), halign="center", size_hint_y=0.05)
        layout.add_widget(self.stats)
        
        layout.add_widget(MDLabel(text="Riy Agent", font_style="H5", theme_text_color="Custom", text_color=(0.4,0.9,1.0,1), halign="center", size_hint_y=0.1))
        ...
        self.input = MDTextField(hint_text="Type...", mode="rectangle", size_hint_x=0.7, pos_hint={"center_y": 0.5})
        self.input.bind(on_text_validate=self.send)
        btn = MDFlatButton(text="Send", md_bg_color=(0.2,0.7,0.9,1), text_color=(1,1,1,1), size_hint_x=0.3, pos_hint={"center_y": 0.5})
    ...
    def add_msg(self, text, is_user, skip=False):
        self.lang = 'ar' if any(c in text for c in 'ابتثجحخدذرزسشصضطظعغفقكلمنهوي') else 'en'
        color = (1,1,1,1) if is_user else (0.4,0.9,1.0,1)
        lbl = MDLabel(text=text, font_style="Body1", theme_text_color="Custom", text_color=color, halign="right" if is_user else "left", size_hint_y=None, padding=[dp(15),dp(10)])
        lbl.texture_update()
    ...
    def process(self, text):
        ...
        if low.startswith(("forget", "انس", "انسي")):
            ...
            self.add_msg("Forgot " + str(d) + " item" if d else "Nothing found", False)
            return
